...
func createUser(name: String, password: String, premium: Bool, newsletter: Bool) -> Result<User, UserError> {
    ...
    let result: Result<User, UserError> = Result.pure(newsletter).apply(Result.pure(premium).apply(Validators.Password(password).apply(Validators.Name(name).map(curry(User.init)))))
